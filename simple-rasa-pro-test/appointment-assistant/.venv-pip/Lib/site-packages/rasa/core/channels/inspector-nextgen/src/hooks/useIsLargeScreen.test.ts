...
function mockMatchMedia(matches: boolean) {
  ...
  const matchMedia = vi.spyOn(window, "matchMedia").mockImplementation(() => mql as MediaQueryList);
  return { mql, listeners, matchMedia };
}
...
describe("useIsLargeScreen", () => {
  ...
  it("uses the embedded breakpoint when isEmbedded is true", () => {
    ...
    const { result } = renderHook(() => useIsLargeScreen());
    expect(result.current).toBe(false);
    expect(matchMedia).toHaveBeenCalledWith("(min-width: 1920px)");
  });
