...
function resolveSocketIoPath(projectUrl: string, socketEndpoint?: string): string | undefined {
  if (socketEndpoint) {
    return socketEndpoint;
  }

  const urlObject = new URL(projectUrl);
  return urlObject.pathname === "/" ? undefined : `${urlObject.pathname}/socket.io`;
}
...
function generatePlaceholderUserUtterance(message: string, buttonTitle?: string): Utterance {
  ...
  return {
    ...
    metadata: {
      parseData: {},
      ...(buttonTitle
        ? { user_message_type: "button_click" as const, button_title: buttonTitle }
        : {}),
    },
  };
}
...
export function useBotConnection({
  ...
  const {
    ...
    httpAuth: s.httpAuth,
    socketEndpoint: s.socketEndpoint,
    socketAuth: s.socketAuth,
  }));
  ...
  const sendMessage = useCallback(
    (message: string, buttonTitle?: string) => {
      ...
      const userUtterance: Utterance = generatePlaceholderUserUtterance(message, buttonTitle);
  ...
  const startSession = useCallback(
    (sessionIdToStart: string) => {
      onSessionStartRef.current?.(sessionIdToStart);
  ...
  const handleTrackerResponse = useCallback(
    (response: TrackerResponseData) => {
      ...
      if (response.sender_id !== sessionId) {
        return;
      }
      ...
      const sessionEnded = events.some(
        (e) =>
          isConversationEvent(e) && e.conversationEventType === ConversationEventType.SessionEnded,
      );
      if (sessionEnded) {
        if (activeModalityRef.current === "voice") {
  ...
  useEffect(() => {
    ...
    if (!socket.current && url) {
      ...
      const socketIoPath = resolveSocketIoPath(url, socketEndpoint);
          ...
          if (activeModalityRef.current === "text") {
            if (!externalSessionId) {
              ...
              getConversationHistory({
                ...
                .then((trackerResult) => {
                  const rawEvents = trackerResult?.events ?? [];
                  if (rawEvents.length === 0) {
                    startSession(externalSessionId);
                    return undefined;
                  }
                  return handleTrackerResponse(trackerResult);
                })
                .catch((err) => {
                  logError(err);
                  return undefined;
