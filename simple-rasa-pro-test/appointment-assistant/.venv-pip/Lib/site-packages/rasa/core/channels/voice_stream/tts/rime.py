...
class _RimeAudioTranscoder:
    def __init__(self, audio_format: AudioFormat) -> None:
        self.audio_format = audio_format
        self._incomplete_audio = b""
        self._ratecv_state: Any = None

    def transcode(self, audio_bytes: bytes) -> bytes:
        if self.audio_format is not L16_48KHZ:
            return audio_bytes

        complete_audio = self._consume_complete_pcm_frames(audio_bytes)
        if not complete_audio:
            return b""

        transcoded_audio, self._ratecv_state = audioop.ratecv(
            complete_audio,
            self.audio_format.bit_depth // 8,
            self.audio_format.channels,
            RIME_PCM_SAMPLE_RATE,
            self.audio_format.sample_rate,
            self._ratecv_state,
        )
        return transcoded_audio

    def warn_if_incomplete_frame(self) -> None:
        if self._incomplete_audio:
            structlogger.warning(
                "rime.stream_audio.incomplete_pcm_frame",
                byte_count=len(self._incomplete_audio),
            )

    def _consume_complete_pcm_frames(self, audio_bytes: bytes) -> bytes:
        audio_bytes = self._incomplete_audio + audio_bytes
        frame_size = (self.audio_format.bit_depth // 8) * self.audio_format.channels
        complete_audio_size = len(audio_bytes) - (len(audio_bytes) % frame_size)
        self._incomplete_audio = audio_bytes[complete_audio_size:]
        return audio_bytes[:complete_audio_size]
...
class RimeTTS(TTSEngine[RimeTTSConfig]):
    ...
    def get_websocket_url(self) -> str:
        ...
        if self.audio_format is L16_24KHZ:
            ...
            sample_rate = RIME_PCM_SAMPLE_RATE
            encoding = "pcm"
    ...
    async def stream_audio(self) -> AsyncIterator[RasaAudioBytes]:
        ...
        transcoder = _RimeAudioTranscoder(self.audio_format)
        ...
        try:
            async for msg in self.ws:
                ...
                if msg_type == "chunk":
                    ...
                    if base64_audio:
                        audio_bytes = transcoder.transcode(
                            base64.b64decode(base64_audio)
                        )
                        if audio_bytes:
                            yield RasaAudioBytes(audio_bytes, format=self.audio_format)

                elif msg_type == "done":
                    # All audio has been sent, stop streaming
                    transcoder.warn_if_incomplete_frame()
                    break
    ...
    def transcode_audio(self, audio_bytes: bytes) -> bytes:
        return _RimeAudioTranscoder(self.audio_format).transcode(audio_bytes)
