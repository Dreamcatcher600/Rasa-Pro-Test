...
from rasa.core.information_retrieval.faiss import STORE_JSON_VERSION, FAISS_Store
...
class EnterpriseSearchPolicy(LLMHealthCheckMixin, EmbeddingsHealthCheckMixin, Policy):
    ...
    def fingerprint_addon(cls, config: Dict[str, Any]) -> Optional[str]:
        ...
        fingerprint_data: List[Any] = [
            prompt_template,
            local_knowledge_data,
            llm_config,
            embedding_config,
        ]
        # Only appended for the in-model FAISS store, so remote vector stores keep
        # their existing cache entries.
        if parsed_config.vector_store_type == DEFAULT_VECTOR_STORE_TYPE:
            fingerprint_data.append(STORE_JSON_VERSION)

        return deep_container_fingerprint(fingerprint_data)
    ...
    @classmethod
    def get_system_default_prompt_based_on_config(
        cls,
        config: Dict[str, Any],
        ...
        Args:
            ...
            model_groups: Model groups from endpoints.yml. Used when Configuration
                endpoints are empty (Studio / model-service).

        Returns:
            The resolved jinja prompt template as a string.
        """
        parsed_config = EnterpriseSearchPolicyConfig.from_dict(
