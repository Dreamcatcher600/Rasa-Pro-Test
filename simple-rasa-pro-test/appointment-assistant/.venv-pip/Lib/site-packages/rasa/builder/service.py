...
@openapi.parameter(
    HEADER_USER_ID,
    description=(
        "User id used to scope guardrail violations (X-User-Id). Required only "
        "when guardrail blocking is enabled (GUARDRAILS_ENABLE_BLOCKING=true), "
        "which stores strikes and blocks per user."
    ),
    _in="header",
    required=False,
    schema=str,
...
async def copilot_v2(request: Request) -> Union[None, HTTPResponse]:
    ...
    try:
        from rasa.builder.copilot.preflight import (
            guardrails_preflight,
            stream_violation_sse,
        )
    ...
    try:
        ...
        # Screen the request before the agent loop can run. The pre-flight is a
        # host concern: it decides policy, the runtime only executes what
        # survives it.
        checker = llm_service.guardrails_policy_checker
        user_id = (request.headers.get(HEADER_USER_ID) or "").strip()
        # Only blocking needs an identity. Strikes and block scopes are stored
        # per user and the store rejects an empty id, so without this the run
        # would fail deeper in with an opaque error. With blocking off nothing
        # is recorded, and a blank header degrades to None like anywhere else.
        if checker is not None and GUARDRAILS_ENABLE_BLOCKING and not user_id:
            structlogger.error(
                "bot_builder_service.copilot_v2.missing_or_empty_user_id_header",
                event_info=(
                    f"Guardrails blocking is enabled, which requires header "
                    f"{HEADER_USER_ID} to scope violations."
                ),
            )
            await sse.send(
                encode_sse_event(
                    ag_ui_events.run_error(
                        f"Missing or empty required header: {HEADER_USER_ID}",
                        code="MissingUserId",
                    )
                )
            )
        ...
        outcome = await guardrails_preflight(checker)(run_request, user_id=user_id)
        if outcome.violation is not None:
            async for frame in stream_violation_sse(
                outcome.violation,
                outcome.run_request,
                thread_store=_get_copilot_v2_thread_store(request),
            ):
        ...
        # Tracker-context resolver: closes over the app and the (optional)
        # X-User-Id header. Resolution happens inside the stream, after
        # RUN_STARTED, so the open SSE connection is never silent while the
        # tracker store or guardrails round-trips.
        app = request.app

        async def _resolve_tracker_context(
            session_id: Optional[str],
        ) -> Optional[TrackerContext]:
            return await _get_sanitized_tracker_context(
                app, session_id, user_id or None
            )
        ...
        async for frame in stream_copilot_v2_sse(
            ...
            project_generator=project_generator,
            email=email,
            tracker_context_resolver=_resolve_tracker_context,
        ):
            await sse.send(frame)
    except Exception as exc:
...
def _get_copilot_v2_thread_store(request: Request) -> "SQLiteThreadStore":
    ...
    """
    from rasa.builder.copilot.v2_thread_history import open_copilot_v2_thread_store

    store = getattr(request.app.ctx, "copilot_v2_thread_store", None)
    if store is None:
        store = open_copilot_v2_thread_store()
...
async def _get_sanitized_tracker_context(
    app: Any,
    session_id: Optional[str],
    user_id: Optional[str],
) -> Optional[TrackerContext]:
    """Load, trim, and guardrail-sanitize the tracker context for one session.

    Keyed on (app, session_id, user_id) — not session_id alone — because the
    guardrails call needs the hello-rasa user id. When no guardrails checker is
    configured the context passes through unsanitized (v1 parity). Missing or
    stale sessions degrade to ``None`` at every layer.

    Args:
        app: The Sanic app owning the agent / tracker store.
        session_id: The Inspector conversation session id, if any.
        user_id: The hello-rasa user id used by the guardrails check, or
            ``None`` when the request carried no usable ``X-User-Id`` header.

    Returns:
        The (sanitized) tracker context, or ``None`` when unavailable.
    """
    if not session_id:
        return None
    tracker = await current_tracker_from_input_channel(app, session_id)
...
async def get_tracker_context_for_copilot(
    ...
    """Resolve the sanitized tracker context for a v1 copilot turn.

    Thin wrapper over ``_get_sanitized_tracker_context`` that keeps the v1
    call sites' request-shaped signature and traces the outcome to Langfuse.

    Args:
        request: The Sanic request owning the app the tracker is read from.
        req: The copilot turn request carrying the Inspector session id.
        user_id: The hello-rasa user id forwarded to the guardrails check.

    Returns:
        The sanitized tracker context, or ``None`` when the request has no
        session id or the session's tracker is unavailable.
    """
    if not req.session_id:
        return None
    tracker_context = await _get_sanitized_tracker_context(
        request.app, req.session_id, user_id
    )
