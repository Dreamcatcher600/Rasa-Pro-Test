...
from rasa.builder.copilot.v2_thread_history import (
    persist_copilot_message_to_v2_thread,
    persist_user_message_to_v2_thread,
)

structlogger = structlog.get_logger()


def _text_from_content_blocks(blocks: List[ContentBlock]) -> str:
    """Join the text blocks of a message, ignoring every richer block type.

    Copilot v2 history rows hold text only, even for the runtime's own turns.
    """
    return "\n\n".join(
...
async def persist_copilot_message_to_history(
    ...
    try:
        ...
        await persist_copilot_message_to_v2_thread(
            _text_from_content_blocks(message_content),
            chat_id,
            response_category=response_category,
        )
...
async def persist_user_message_to_history(
    ...
    try:
        ...
        await persist_user_message_to_v2_thread(text, chat_id)
