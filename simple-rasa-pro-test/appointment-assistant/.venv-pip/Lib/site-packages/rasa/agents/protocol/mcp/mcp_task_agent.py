...
class MCPTaskAgent(MCPBaseAgent):
    ...
    async def send_message(
        ...
        unavailable_tool_call_retries: int = 0
        last_unavailable_tool_name: Optional[str] = None

        for iteration in range(self.MAX_ITERATIONS):
            current_iteration_tool_results: Dict[str, AgentToolResult] = {}
            had_unavailable_tool_call: bool = False
            try:
                ...
                # Repair provider-mangled tool names before any name is read, so
                # that slot-setting tools and the assistant message echoed back to
                # the LLM both see the corrected name.
                self._normalize_tool_call_names(
                    llm_response, _available_tools_names, "mcp_task_agent"
                )
                ...
                if llm_response.tool_calls:
                    ...
                    for tool_call in llm_response.tool_calls:
                        ...
                        # If the tool is not available, answer the tool call with
                        # an error and let the LLM correct itself on the next
                        # iteration instead of failing the whole agent.
                        if tool_call.tool_name not in _available_tools_names:
                            self._handle_unavailable_tool_call(
                                tool_call,
                                _available_tools_names,
                                tool_call_messages,
                                "mcp_task_agent",
                            )
                            last_unavailable_tool_name = tool_call.tool_name
                            had_unavailable_tool_call = True
                            continue
                ...
                # Give up once the LLM keeps naming tools that do not exist. Checked
                # after the exit conditions so a task that is genuinely complete is
                # not failed because the same response also named a missing tool.
                # Also fail on the last iteration: there is no remaining turn for
                # the corrective retry, so treat this like a failed retry instead
                # of falling through to max-iterations COMPLETED.
                if had_unavailable_tool_call:
                    unavailable_tool_call_retries += 1
                    is_last_iteration = iteration == self.MAX_ITERATIONS - 1
                    if (
                        unavailable_tool_call_retries
                        > self.MAX_UNAVAILABLE_TOOL_CALL_RETRIES
                        or is_last_iteration
                    ):
                        return self._create_fatal_error_output(
                            agent_input,
                            f"Tool {last_unavailable_tool_name} is not available.",
                            ...
                            tool_results=tool_results,
                            tool_name=last_unavailable_tool_name,
                        )
