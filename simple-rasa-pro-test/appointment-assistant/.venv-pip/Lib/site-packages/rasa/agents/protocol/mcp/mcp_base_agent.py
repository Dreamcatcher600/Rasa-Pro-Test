...
# Longest run of characters that can legally open a tool name. Providers can leak
# serialisation debris into the name field (e.g. Bedrock returning
# `task_completed" />`); the debris always falls outside this character class.
_LEADING_TOOL_NAME_PATTERN = re.compile(r"[A-Za-z0-9_.\-]+")

# Characters a tool name may contain before providers start rejecting it.
_PROVIDER_SAFE_TOOL_NAME_PATTERN = re.compile(r"[^A-Za-z0-9_\-]")

# OpenAI's tool name limit. Also bounds how much of a mangled name is logged.
_MAX_TOOL_NAME_LENGTH = 64

# Stand-in used when a mangled name sanitises away to nothing, so the message
# echoed back to the provider still carries a legal tool name.
_PLACEHOLDER_TOOL_NAME = "unknown_tool"

# Above this many tools the corrective message stops enumerating them.
_MAX_ENUMERATED_TOOL_NAMES = 30


def repair_malformed_tool_name(
    raw_tool_name: str, available_tool_names: List[str]
) -> Optional[str]:
    """Return the available tool name that a malformed tool name repairs to.

    Some providers leak tool-use serialisation debris into the tool name field,
    e.g. Bedrock via LiteLLM returning `task_completed" />` in place of
    `task_completed`. A repair is accepted only when the discarded suffix is
    markup-like (no alphanumeric characters) and the remaining identifier is
    exactly an available tool, so a repair can never invent a tool nor turn one
    legitimate tool name into a different legitimate one.

    Args:
        raw_tool_name: The tool name exactly as returned by the LLM provider.
        available_tool_names: Names of the tools offered in this request.

    Returns:
        The repaired name, or `None` when the name is already valid or no safe
        repair exists.
    """
    # Load-bearing: returning early for an already-valid name is what guarantees
    # a legitimate tool name is never rewritten into another one.
    if raw_tool_name in available_tool_names:
        return None

    match = _LEADING_TOOL_NAME_PATTERN.match(raw_tool_name.strip())
    if match is None:
        return None

    candidate = match.group(0)
    discarded_suffix = raw_tool_name.strip()[match.end() :]
    # An alphanumeric suffix is a plausible part of a different tool name (e.g.
    # `set_slot_amount confirmed`), so truncating it could silently dispatch the
    # wrong tool. Only pure punctuation is treated as debris.
    if any(character.isalnum() for character in discarded_suffix):
        return None

    return candidate if candidate in available_tool_names else None
...
class MCPBaseAgent(AgentProtocol):
    ...
    # How many times the LLM may name an unavailable tool before the agent gives
    # up. One corrective retry, then the run fails as it did before.
    MAX_UNAVAILABLE_TOOL_CALL_RETRIES = 1
    ...
    @staticmethod
    def messages_contain_tool_exchange(messages: List[Dict[str, Any]]) -> bool:
        """Whether any message carries a tool call or a tool result.

        Providers such as AWS Bedrock reject a request that contains tool-use or
        tool-result content blocks unless a (non-empty) tool config is also sent.
        Callers use this to decide whether tools may be withheld from a request.
        """
        return any(
            message.get(KEY_TOOL_CALLS) or message.get(KEY_ROLE) == ROLE_TOOL
    ...
    def _normalize_tool_call_names(
        self,
        llm_response: LLMResponse,
        available_tool_names: List[str],
        component_name: str,
    ) -> None:
        """Repair provider-mangled tool names on `llm_response`, in place.

        Runs before any tool name is read so that the repaired name reaches every
        consumer, including the assistant message echoed back to the provider.
        Names that cannot be repaired to an available tool are still sanitised to
        a provider-legal form; they stay unavailable and are never dispatched.

        Args:
            llm_response: The response whose tool calls are normalised in place.
            available_tool_names: Names of the tools offered in this request.
            component_name: Agent component name used to prefix log events.
        """
        agent_id = str(make_agent_identifier(self._name, self.protocol_type))
        for tool_call in llm_response.tool_calls or []:
            raw_tool_name = tool_call.tool_name
            # A name the LLM got right is never touched, by either branch below.
            if raw_tool_name in available_tool_names:
                continue

            repaired_tool_name = repair_malformed_tool_name(
                raw_tool_name, available_tool_names
            )
            if repaired_tool_name is not None:
                tool_call.tool_name = repaired_tool_name
                structlogger.warning(
                    f"{component_name}.send_message.repaired_malformed_tool_name",
                    event_info=(
                        "The LLM provider returned a malformed tool name. Repaired "
                        "it to an available tool before dispatching."
                    ),
                ...
                structlogger.debug(
                    f"{component_name}.send_message.repaired_malformed_tool_name.raw",
                    event_info="The malformed tool name as returned by the provider.",
                    raw_tool_name=raw_tool_name[:_MAX_TOOL_NAME_LENGTH],
                    tool_name=repaired_tool_name,
                    agent_name=self._name,
                    agent_id=agent_id,
                )
            ...
            provider_safe_tool_name = self._sanitize_tool_name_for_provider(
                raw_tool_name, available_tool_names
            )
            if provider_safe_tool_name != raw_tool_name:
                # The name stays unavailable either way, but echoing the raw value
                # back would be rejected by providers that validate tool names.
                tool_call.tool_name = provider_safe_tool_name
                structlogger.warning(
                    f"{component_name}.send_message.sanitized_tool_name",
                    event_info=(
                        "The LLM provider returned a tool name that no provider "
                        "would accept. Sanitized it so the retry request stays "
                        "valid. The tool is still unavailable."
                    ),
                    tool_name=provider_safe_tool_name,
                    agent_name=self._name,
                    agent_id=agent_id,
                )
                structlogger.debug(
                    f"{component_name}.send_message.sanitized_tool_name.raw",
                    event_info="The unusable tool name as returned by the provider.",
                    raw_tool_name=raw_tool_name[:_MAX_TOOL_NAME_LENGTH],
                    tool_name=provider_safe_tool_name,
                    agent_name=self._name,
                    agent_id=agent_id,
    ...
    def _sanitize_tool_name_for_provider(
        self, tool_name: str, available_tool_names: List[str]
    ) -> str:
        """Return a provider-legal tool name that no available tool answers to.

        Only ever called for a name that is already known to be unavailable.
        Stripping illegal characters can join fragments into a real tool name
        (`get_bal ance` -> `get_balance`), which would dispatch a tool the LLM
        never named, using the arguments it sent for a different one. Any such
        collision falls back to a placeholder instead, so the name stays
        unavailable and takes the corrective retry path.

        Args:
            tool_name: The unavailable tool name as returned by the provider.
            available_tool_names: Names of the tools offered in this request.

        Returns:
            A name matching `[A-Za-z0-9_-]{1,64}` that is not an available tool.
        """
        sanitized = _PROVIDER_SAFE_TOOL_NAME_PATTERN.sub("", tool_name)[
            :_MAX_TOOL_NAME_LENGTH
        ]
        if sanitized and sanitized not in available_tool_names:
            return sanitized

        placeholder = _PLACEHOLDER_TOOL_NAME
        collision_count = 0
        while placeholder in available_tool_names:
            collision_count += 1
            placeholder = f"{_PLACEHOLDER_TOOL_NAME}_{collision_count}"
        return placeholder

    def _handle_unavailable_tool_call(
        self,
        ...
        """Answer an unavailable tool call with an error so the LLM can retry.

        Appends a tool result for `tool_call.id` rather than failing the run. The
        result is required regardless: OpenAI-compatible providers reject an
        assistant message whose tool calls are not all answered.

        Args:
            tool_call: The tool call naming a tool that is not available.
            available_tool_names: Names of the tools offered in this request.
            tool_call_messages: Conversation messages the result is appended to.
            component_name: Agent component name used to prefix log events.
        """
        structlogger.warning(
            f"{component_name}.send_message.tool_not_available",
            event_info=(
                "The LLM requested a tool that is not available. Answering the "
                "tool call with an error so the LLM can retry."
            ),
        ...
        self._append_tool_result_message(
            tool_call_messages,
            tool_call.id,
            self._get_tool_result_for_unavailable_tool(
                tool_call.tool_name, available_tool_names
            ),
        )

    def _get_tool_result_for_unavailable_tool(
        self, tool_name: str, available_tool_names: List[str]
    ) -> str:
        """Build the corrective tool result content for an unavailable tool."""
        if not available_tool_names:
            guidance = "No tools are available."
        elif len(available_tool_names) > _MAX_ENUMERATED_TOOL_NAMES:
            guidance = "Use only the tools provided in this request."
        else:
            guidance = f"Available tools: {', '.join(sorted(available_tool_names))}."
        return (
            f"Error: `{tool_name}` is not an available tool and was not executed. "
            f"{guidance} Call an available tool using its exact name, or answer "
            "the user directly without calling a tool."
        )
