...
def link_or_call_frames_hosting_interrupted_agent(
    dialogue_stack: DialogueStack,
) -> List[UserFlowStackFrame]:
    """Return LINK/CALL user frames that host an interrupted or resuming agent.

    Default continuable-frame discovery skips LINK/CALL frames because they are
    treated as part of a calling flow. When a digression interrupts a sub-agent
    under a pure LINK/CALL stack, that skip leaves no continuable candidates and
    ``pattern_continue_interrupted`` never fires (ENG-3058). Use this helper as a
    fallback only when the default list is empty.

    Args:
        dialogue_stack: The dialogue stack.

    Returns:
        LINK/CALL user frames whose ``flow_id`` matches an agent frame in
        ``INTERRUPTED`` or ``RESUMING`` state.
    """
    agent_host_flow_ids = {
        frame.flow_id
        for frame in dialogue_stack.frames
        if isinstance(frame, AgentStackFrame)
        and frame.state in (AgentState.INTERRUPTED, AgentState.RESUMING)
    }
    return [
        frame
        for frame in dialogue_stack.frames
        if isinstance(frame, UserFlowStackFrame)
        and frame.frame_type in (FlowStackFrameType.LINK, FlowStackFrameType.CALL)
        and frame.flow_id in agent_host_flow_ids
    ]
