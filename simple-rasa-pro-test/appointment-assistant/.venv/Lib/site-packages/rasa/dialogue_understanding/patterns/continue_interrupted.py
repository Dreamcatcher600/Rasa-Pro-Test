...
class ActionCancelInterruptedFlows(Action):
    ...
    def _collect_frames_to_cancel(
        ...
        # Collect the target user-flow frame plus frames above it (agent,
        # pattern, …) until the next standalone user-flow boundary.
        #
        # ENG-3058: LINK/CALL frames are normally not standalone units, but when
        # one hosts an interrupted agent it must be allowed to start the group.
        agent_hosting_frame_ids = {
            frame.frame_id
            for frame in link_or_call_frames_hosting_interrupted_agent(stack)
        }

        for frame in stack.frames:
            if isinstance(frame, UserFlowStackFrame):
                # Boundaries: where a cancel group may start or must stop.
                # Only REGULAR/INTERRUPT — not LINK/CALL — so a linked child
                # above a parent does not cut collection short.
                is_cancel_boundary = (
                    frame.frame_type == FlowStackFrameType.REGULAR
                    or frame.frame_type == FlowStackFrameType.INTERRUPT
                )
                # Anchors: boundaries, plus agent-hosting LINK/CALL (ENG-3058).
                can_anchor_cancel = (
                    is_cancel_boundary or frame.frame_id in agent_hosting_frame_ids
                )

                if can_anchor_cancel and frame.flow_id == target_flow_id:
                    # Start of the group to cancel.
                    frames_to_cancel.append(frame)
                    frame_to_cancel = frame
                    frame_found = True
                    continue
                elif is_cancel_boundary and frame_found:
                    # Next standalone user flow — end of this cancel group.
                    break
