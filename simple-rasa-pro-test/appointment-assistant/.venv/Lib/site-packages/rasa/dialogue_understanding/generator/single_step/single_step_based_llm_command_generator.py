...
from rasa.core.information_retrieval.faiss import STORE_JSON_VERSION
...
class SingleStepBasedLLMCommandGenerator(LLMBasedCommandGenerator, ABC):
    ...
    def fingerprint_addon(cls: Any, config: Dict[str, Any]) -> Optional[str]:
        ...
        fingerprint_data: List[Any] = [prompt_template, llm_config, embedding_config]
        # Only appended when flow retrieval persists a FAISS index, so assistants
        # without one keep their existing cache entries.
        if config.get(FLOW_RETRIEVAL_KEY, {}).get(FLOW_RETRIEVAL_ACTIVE_KEY, True):
