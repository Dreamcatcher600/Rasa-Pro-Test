...
def collect_frames_to_resume(
    ...
    # one hosts an interrupted agent it must be allowed to start the resume group.
    agent_hosting_frame_ids = {
        frame.frame_id for frame in link_or_call_frames_hosting_interrupted_agent(stack)
    }

    for frame in stack.frames:
        if isinstance(frame, UserFlowStackFrame):
            # Boundaries: where a resume group may start or must stop.
            ...
            can_anchor_resume = (
                is_resume_boundary or frame.frame_id in agent_hosting_frame_ids
            )

            if can_anchor_resume and frame.flow_id == target_flow_id:
                # Start of the group to resume.
                frames_to_resume.append(frame)
                frame_to_resume = frame
                frame_found = True
                continue
            elif is_resume_boundary and frame_found:
                # Next standalone user flow — end of this resume group.
                break
