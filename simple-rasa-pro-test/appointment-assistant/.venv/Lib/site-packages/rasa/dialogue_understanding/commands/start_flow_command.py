...
class StartFlowCommand(Command):
    ...
    def run_command_on_tracker(
        ...
        # Default top-user discovery skips LINK/CALL. Under a pure LINK/CALL
        # stack that hosts an interrupted/resuming agent, fall back so StartFlow
        # can still resume that flow or push further digressions as INTERRUPT
        # (ENG-3058).
        #
        # The helper matches INTERRUPTED/RESUMING hosts only, so on the *first*
        # digression (agent still WAITING_FOR_INPUT) it finds nothing and
        # `original_top_flow` stays None: no FlowInterrupted is emitted for the
        # host flow, while a later digression does emit one. That asymmetry is
        # accepted rather than fixed by widening the helper, which would make
        # pattern_continue_interrupted treat a still-waiting agent as
        # interrupted. Interrupting the agent does not depend on this fallback
        # (see the unconditional block below), and FlowInterrupted has no
        # dialogue-logic consumers, so the gap is telemetry only.
        original_user_frame = top_user_flow_frame(original_stack)
        if original_user_frame is None:
            agent_host_frames = link_or_call_frames_hosting_interrupted_agent(
                original_stack
            )
            if agent_host_frames:
                original_user_frame = agent_host_frames[-1]
        original_top_flow = (
        ...
        # If there is an active agent frame, interrupt it. This must run
        # independently of `original_top_flow`: the agent-calling flow may sit
        # under a LINK/CALL frame that `top_user_flow_frame()` skips, leaving
        # `original_top_flow` None even though a sub-agent is waiting for input.
        # This mirrors KnowledgeAnswer/Clarify/ChitChat, which interrupt the
        # active agent unconditionally (ENG-3058).
        active_agent_stack_frame = stack.find_active_agent_frame()
        if active_agent_stack_frame:
            # a digression that interrupts an agent must be pushed as an INTERRUPT
            # frame so the continue_interrupted pattern fires when it completes
            frame_type = FlowStackFrameType.INTERRUPT
            structlogger.debug(
