...
class TrainingSession(BaseModel):
    ...
    auto_deploy_error: Optional[str] = None
...
_DENYLISTED_DIRNAMES = frozenset({"__pycache__", ".rasa", "models"})


def read_bot_configs_from_dir(base_path: str) -> Dict[str, bytes]:
    """Read bot config and assistant files from a directory as raw bytes.

    Returns a dict keyed by short name for the three main config files
    (``"config"``, ``"credentials"``, ``"endpoints"``) and by relative path for
    every other non-denylisted file in the assistant tree (e.g. ``db/``,
    ``actions/``, ``prompts/``, ``docs/``).

    Denylisted paths are not snapshotted: ``__pycache__``, ``.rasa``, ``models``,
    any directory or file whose name starts with ``.``, and ``*.pyc`` files.
    """
    configs: Dict[str, bytes] = {}
    for filename, key in ROOT_CONFIG_FILES.items():
        path = os.path.join(base_path, filename)
        if os.path.exists(path):
            with open(path, "rb") as f:
                configs[key] = f.read()

    _read_project_tree_files(base_path, configs)
    return configs


def _is_denylisted_dirname(dirname: str) -> bool:
    """Return whether a directory should be excluded from the deploy snapshot."""
    return dirname in _DENYLISTED_DIRNAMES or dirname.startswith(".")


def _is_denylisted_filename(filename: str) -> bool:
    """Return whether a file should be excluded from the deploy snapshot."""
    return filename.endswith(".pyc") or filename.startswith(".")


def _read_project_tree_files(base_path: str, configs: Dict[str, bytes]) -> None:
    """Read all non-denylisted files under base_path into configs.

    Root ``config.yml`` / ``credentials.yml`` / ``endpoints.yml`` are skipped here
    because they are already stored under short keys. Every other relative path is
    validated with ``validate_relative_bot_path`` so unsafe names fail during
    snapshot instead of later at deploy write.
    """
    for root, dirnames, files in os.walk(base_path):
        dirnames[:] = [
            dirname for dirname in dirnames if not _is_denylisted_dirname(dirname)
        ]
        for fname in files:
            if _is_denylisted_filename(fname):
                continue
            if root == base_path and fname in ROOT_CONFIG_FILES:
                continue
            fpath = os.path.join(root, fname)
            rel = os.path.relpath(fpath, base_path).replace("\\", "/")
            validate_relative_bot_path(rel)
