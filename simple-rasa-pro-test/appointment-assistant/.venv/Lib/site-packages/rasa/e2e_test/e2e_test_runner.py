...
class E2ETestRunner:
    ...
    def merge_metadata(
        ...
        if not test_case_metadata:
            # Return a copy so runtime-injected keys (model_id, assistant_id,
            # session_id) written by the caller never leak into the shared,
            # per-invocation parsed metadata dict (PF-1637).
            return copy.deepcopy(step_metadata)
        if not step_metadata:
            return copy.deepcopy(test_case_metadata)
