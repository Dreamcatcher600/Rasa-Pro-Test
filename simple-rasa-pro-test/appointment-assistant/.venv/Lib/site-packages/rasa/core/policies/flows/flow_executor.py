...
from rasa.dialogue_understanding.stack.utils import (
    link_or_call_frames_hosting_interrupted_agent,
...
def _continuable_user_flow_frames(
    frames: List[UserFlowStackFrame],
    flows: FlowsList,
) -> List[UserFlowStackFrame]:
    """Return user-flow frames that still have steps left to run."""
    continuable: List[UserFlowStackFrame] = []
    for frame in frames:
        step = frame.step(flows)
        flow = frame.flow(flows)
        if step is not None and flow is not None and not is_step_end_of_flow(step):
            continuable.append(frame)
    return continuable
...
def trigger_pattern_continue_interrupted(
    ...
    # Default scan skips LINK/CALL frames (treated as part of the calling flow).
    interrupted_user_flows_to_continue = _continuable_user_flow_frames(
        user_frames_on_the_stack(stack), flows
    )

    # ENG-3058: when only a LINK/CALL frame hosts an interrupted agent, the default
    # scan finds nothing. Fall back so pattern_continue_interrupted can still offer
    # resume for that agent-calling flow.
    #
    # This is deliberately a last resort — a continuable REGULAR/INTERRUPT parent
    # always wins. That is safe: resuming such a parent still sweeps the LINK/CALL
    # frame and the agent frame into `frames_to_resume`, because LINK/CALL frames
    # are not resume boundaries and collection does not stop at them, so the agent
    # still reaches RESUMING. Only the prompt wording differs, naming the outer
    # flow rather than the agent-calling one.
    if not interrupted_user_flows_to_continue:
        interrupted_user_flows_to_continue = _continuable_user_flow_frames(
            link_or_call_frames_hosting_interrupted_agent(stack), flows
        )
    ...
    flow_ids = [frame.flow_id for frame in interrupted_user_flows_to_continue]
    continue_interrupted_frame = ContinueInterruptedPatternFlowStackFrame(
        ...
        interrupted_flow_options=assemble_options_string(flow_names),
    )

    # Push below a top LINK frame so the linked flow runs first (part of the
    # current flow) before continue_interrupted asks the user.
    ...
    stack.push(
        continue_interrupted_frame,
        index=-1 if push_below_link else None,
    )
...
def _silence_timeout_events_for_collect_step(
    ...
    if step.silence_timeout:
        ...
        input_channel_name = tracker.get_latest_input_channel()
        channel_config = _get_channel_config_from_credentials(input_channel_name)
...
def _get_channel_config_from_credentials(
    input_channel_name: Optional[str],
) -> Optional[Dict[str, Any]]:
    """Look up channel config from credentials for the given channel name.

    For custom channels keyed by their full module path (e.g.,
    "my.custom.channel.MyCustomChannel"), this function tries both an exact
    match and a suffix match with the simple channel name.

    Args:
        input_channel_name: The channel name to look up (e.g., "slack" or
            "MyCustomChannel" from channel.name()).

    Returns:
        The channel configuration dict if found, None otherwise.
    """
    credentials_config = Configuration.get_instance().credentials

    if not credentials_config or not input_channel_name:
        return None

    # Try exact match first (for built-in channels or custom channels
    # that override name() to return the full module path)
    channel_config = credentials_config.channels.get(input_channel_name)

    # If no exact match, try finding a channel key that ends with the
    # input_channel_name (for custom channels keyed by module path)
    if not channel_config:
        for channel_key in credentials_config.channels:
            if channel_key.endswith(f".{input_channel_name}"):
                channel_config = credentials_config.channels[channel_key]
                break
...
def _get_default_silence_timeout(tracker: DialogueStateTracker) -> float:
    ...
    Looks up the silence_timeout in credentials for the tracker's input channel.

    Args:
        tracker: The dialogue state tracker.

    Returns:
        The silence timeout value in seconds.
    """
    input_channel_name = tracker.get_latest_input_channel()
    channel_config = _get_channel_config_from_credentials(input_channel_name)
