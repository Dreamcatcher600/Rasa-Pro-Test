...
class RasaAudioBytes:
    ...
    def to_pcm16(self) -> bytes:
        """Convert the audio data to PCM16 format."""
        if self.format.encoding == AudioEncoding.MULAW:
            return audioop.ulaw2lin(self.data, 2)
        if self.format.encoding == AudioEncoding.LINEAR and self.format.bit_depth == 16:
            return self.data if len(self.data) % 2 == 0 else self.data + b"\x00"
        raise ValueError(f"Unsupported recording audio format: {self.format}")
