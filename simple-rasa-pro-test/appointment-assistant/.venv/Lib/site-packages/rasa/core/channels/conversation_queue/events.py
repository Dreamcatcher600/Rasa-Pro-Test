...
@dataclass(frozen=True)
class TextInputEvent(VoiceInputEvent):
    """A text message sent directly via the WebSocket (bypassing ASR).

    Used when the client sends typed text instead of audio on a voice channel.

    Attributes:
        text: The user-typed text.
        metadata: Optional metadata from the channel (e.g. call parameters).
    """
